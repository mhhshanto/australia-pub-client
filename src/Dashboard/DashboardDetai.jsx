...
function DashboardDetai() {
  ...
  const handleChangeStatus = () => { 
      setLoading(true)
      axios.patch('https://hasib-vai-backend.vercel.app/dashboard/change-status', { status: status, id: data?._id })
        .then(res => {
          setLoading(false)
          ...
          <div className='w-full'>
            ...
            <div className='w-full grid px-3 xl:grid-cols-12   gap-16 my-14'>
              ...
              <div className="2xl:col-span-4 lg:col-span-6 h-fit flex max-w-lg border p-3 rounded-md flex-col gap-1 mb-5 w-full">
                ...
                <div className='mt-10 flex lg:gap-12 gap-6 justify-between'>
                  <Select onChange={(e) => setStatus(e.target.value)} className='flex-1' id="countries" required>
                    <option value={'processing'}>Processing</option>
                    <option value={'cancel'}>Cancel</option>
                    <option value={'delivery'}>Delivery</option>
                  </Select>
                  {
                    loading ?<Button isProcessing >Change Status</Button> :
